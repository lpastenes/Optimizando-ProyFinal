...
const btnPasajero = e => {
    ...
    if (e.target.classList.contains('btn-secondary')) {
        ...
        productoCarro.cantidad === 0 && delete carrito[e.target.dataset.id] // REDUCCION DE UNA CONDICIONAL
...
function TerminarCompra() {
    formCompra.innerHTML = '';
    if (Object.keys(carrito).length != 0) {
        templateCompra.getElementById('nombre');
        templateCompra.getElementById('correo');
        templateCompra.getElementById('fono');
        const clone = templateCompra.cloneNode(true);
        fragmento.appendChild(clone);
        formCompra.appendChild(fragmento);

        const btnPago = document.querySelector('#Procesar-Pago')
        btnPago.addEventListener('click', () => {
            Swal.fire({
                title: 'El pago fue procesado',
                text: 'El comprobante de pago sera enviado a tu mail',
                icon: 'success',
